...
function buildZZZ(frame) {
    const opacity = [1, 0.6, 0.3][frame % 3];
    const sizes = ["8", "6", "5"];
    const offsets = [["14", "0"], ["18", "-5"], ["22", "-9"]];
    return offsets.map((o, i) => `<text x="${o[0]}" y="${o[1]}" font-size="${sizes[i]}" fill="#888" opacity="${i === frame % 3 ? 1 : 0.3}">z</text>`).join("");
}
...
function planCatPath(cells) {
    ...
    const sorted = [...active].sort((a, b) => a.weekIndex !== b.weekIndex ? a.weekIndex - b.weekIndex : a.dayIndex - b.dayIndex);
    return sorted;
}
function generateCatSVG(graph, username, theme = "github") {
    ...
    const ratsSVG = [rat1Path, rat2Path].map((path, ri) => buildRatAnimation(path, totalDuration, ri)).join("\n");
    // Month labels
    const monthLabels = buildMonthLabels(weeks, isDark ? "#8b949e" : "#57606a");
    // Day labels
    const dayLabels = ["", "Mon", "", "Wed", "", "Fri", ""].map((d, i) => d ? `<text x="${GRID_LEFT - 5}" y="${GRID_TOP + i * STEP + 9}" font-size="8" text-anchor="end" fill="${isDark ? "#8b949e" : "#57606a"}" font-family="monospace">${d}</text>` : "").join("\n");
    // Title
...
function buildRatAnimation(path, totalDuration, index) {
    const durS = (totalDuration / 1000).toFixed(2);
